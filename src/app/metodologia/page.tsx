...
export const metadata: Metadata = {
  ...
  description:
    "Conoce cómo funciona mi diagnóstico inicial web: revisión clara para detectar qué está frenando tu sitio y qué conviene priorizar primero.",
  alternates: {
...
const principles = [
  {
    ...
    body: "No todo lo medible es prioritario. Leo el contexto antes de decidir.",
  },
  {
  ...
  },
  {
    ...
    body: "Explico qué encontré, por qué importa y qué no vale la pena tocar.",
  },
]
...
export default function MetodologiaPage() {
  ...
  return (
    <>
      <Header />
      <main className="min-h-screen bg-background text-foreground gradient-mesh">
      <section id="diagnostico-inicial" className="mx-auto max-w-5xl px-6 pb-14 pt-24 sm:pb-16 sm:pt-28">
          ...
          />
          <div className="mb-6">
            <span className="inline-flex items-center rounded-full border border-border bg-white px-3 py-1 text-xs font-medium text-primary">
              Audit Studio · Framework
            </span>
          </div>
        <h1 className="font-display text-3xl font-bold leading-tight sm:text-4xl">Diagnóstico inicial de tu sitio web</h1>
        <p className="mt-4 max-w-3xl text-base text-muted-foreground sm:text-lg">
          Una revisión inicial para entender qué está funcionando, qué no y qué conviene priorizar
          primero.
        </p>
        <p className="mt-4 text-sm text-muted-foreground">
          Si luego necesitas una auditoría completa, también te guío en ese siguiente nivel.
        </p>
      ...
      <section className="mx-auto max-w-5xl px-6 pb-12">
        <h2 className="font-display text-2xl font-semibold">Mi página web tarda mucho en cargar: cómo lo diagnostico</h2>
        <div className="mt-4 rounded-2xl border border-border bg-white p-6 shadow-sm">
          <p className="text-sm leading-relaxed text-muted-foreground sm:text-base">
            Cuando un sitio carga lento, no asumo: mido. Reviso recursos pesados, imágenes sin optimizar,
            scripts que bloquean render, caché y estado general de Core Web Vitals. Con eso defino un orden claro
      ...
      <section className="mx-auto max-w-5xl px-6 pb-12">
        <h2 className="font-display text-2xl font-semibold">Principios</h2>
      ...
      <section className="mx-auto max-w-5xl px-6 pb-12">
        <h2 className="font-display text-2xl font-semibold">Proceso</h2>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
      ...
      <section className="mx-auto max-w-5xl px-6 pb-12">
        <h2 className="font-display text-2xl font-semibold">Qué incluye / Qué no incluye el diagnóstico inicial</h2>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <article className="rounded-2xl border border-border bg-white p-6 shadow-sm">
            <h3 className="font-display text-lg font-semibold text-primary">Incluye</h3>
          ...
          <article className="rounded-2xl border border-border bg-white p-6 shadow-sm">
            <h3 className="font-display text-lg font-semibold text-muted-foreground">No incluye (en esta etapa)</h3>
            <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
              {excludes.map((item) => (
                <li key={item} className="flex gap-2">
      ...
      <section className="mx-auto max-w-5xl px-6 pb-12">
        <h2 className="font-display text-2xl font-semibold">Para quién es / Para quién no es</h2>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <article className="rounded-2xl border border-border bg-white p-6 shadow-sm">
            <h3 className="font-display text-lg font-semibold">Para quién es</h3>
            <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
              {forWho.map((item) => (
                <li key={item} className="flex gap-2">
                  <span className="text-primary">•</span>
          ...
          <article className="rounded-2xl border border-border bg-white p-6 shadow-sm">
            <h3 className="font-display text-lg font-semibold">Para quién no es</h3>
            <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
              {notForWho.map((item) => (
                <li key={item} className="flex gap-2">
                  <span className="text-muted-foreground/60">•</span>
      ...
      <section className="mx-auto max-w-5xl px-6 pb-16">
        <article className="rounded-2xl border border-border bg-white p-6 shadow-sm sm:p-8">
          <h2 className="font-display text-2xl font-semibold">Próximo paso</h2>
          <p className="mt-3 text-sm text-muted-foreground sm:text-base">
            Si este diagnóstico inicial te hace sentido, puedo revisar tu caso y definir si
            conviene avanzar a una auditoría técnica completa.
          </p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <Link
              href="#diagnostico-inicial"
              className="inline-flex items-center justify-center rounded-xl border border-primary px-5 py-3 text-sm font-semibold text-primary hover:bg-primary/5"
            >
          ...
          <p className="mt-4 text-xs text-muted-foreground">Sin presión. Te ayudo a priorizar.</p>
        </article>
      </section>
      </main>
      <Footer />
